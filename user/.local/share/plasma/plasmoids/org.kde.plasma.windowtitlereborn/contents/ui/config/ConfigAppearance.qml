...
Kirigami.ScrollablePage {
    ...
    readonly property alias cfg_minLength: minLength.value
    ...
    readonly property alias cfg_horizontalAlignment: alignmentCombo.currentIndex
    readonly property alias cfg_hideWhenUnavailable: hideWhenUnavailableChk.checked

    // lengthKind reference:
    //   0 = Based on contents
    //   1 = Fixed Length
    //   2 = Maximum Length
    //   3 = Minimum Length         (new)
    //   4 = Min + Max Length       (new)
    readonly property bool needsFixedLen: lengthKind.currentIndex === 1 || lengthKind.currentIndex === 2 || lengthKind.currentIndex === 4
    readonly property bool needsMinLen:   lengthKind.currentIndex === 3 || lengthKind.currentIndex === 4
    ...
    Kirigami.FormLayout {
        ...
        PC3.CheckBox {
            id: hideWhenUnavailableChk
            Kirigami.FormData.label: i18n("Hide widget:")
            text: i18n("When title unavailable")
        }
        RowLayout {
        ...
        PC3.ComboBox {
            ...
            // Elide only makes sense when text can be truncated (fixed/max/minMax)
            visible: needsFixedLen
        }
        ...
        Kirigami.Separator {
            Kirigami.FormData.isSection: true
            Kirigami.FormData.label: "Size & Margins"
        }
        PC3.ComboBox {
            id: lengthKind
        ...
        // Fixed / Maximum length slider (kinds 1, 2, 4)
        RowLayout{
            Kirigami.FormData.label: needsFixedLen && needsMinLen
                ? i18n("Maximum length:")
                : i18n("Custom length:")
            visible: needsFixedLen
            PC3.Slider{
        ...
        // Minimum length slider (kinds 3, 4)
        RowLayout{
            Kirigami.FormData.label: i18n("Minimum length:")
            visible: needsMinLen
            PC3.Slider{
                id: minLength
                from: 0
