    ...
    Item {
        ...
        RowLayout {
            ...
            PlasmaComponents.Label {
                ...
                // Preview all 5 length kinds correctly
                Layout.minimumWidth     : {
                    if      (cfg_lengthKind === 0) return implicitWidth
                    else if (cfg_lengthKind === 1) return cfg_fixedLength
                    else if (cfg_lengthKind === 2) return Math.min(implicitWidth, cfg_fixedLength)
                    else if (cfg_lengthKind === 3) return Math.max(implicitWidth, cfg_minLength)
                    else                           return Math.min(Math.max(implicitWidth, cfg_minLength), cfg_fixedLength)
                }
                Layout.maximumWidth     : Layout.minimumWidth
                font {
